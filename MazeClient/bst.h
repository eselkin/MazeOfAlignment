...
private:
    vector< node<T> *> bstroot; // A vector of pointers of node pointers of T type // NULL if no root/child node, etc!
    int lastInserted;

    void nukem();
    void copy(const bst<T> &other);
    void copy(const vector< node<T> *> &v);
...
{
    return find(1,data) == NULL? false:true;
}
...
int bst<T>::find(int from_idx, const T &data)
{
    ...
    if( bstroot[from_idx] && bstroot[from_idx]->data == data)
        return from_idx;
    else
        if(bstroot[from_idx] && bstroot[from_idx]->data > data)
            return find(2*from_idx, data); // RIGHT <, REALLY LEFT
        else if(bstroot[from_idx] && bstroot[from_idx]->data < data)
...
int bst<T>::operator[](const T &data)
{
    try {
        if (find(1, data) <= 0)
        {
            qDebug() << "find: "  << find(1,data) << "to find: " << endl;
            return -1;
        }
        else
...
template<typename T>
void bst<T>::copy(const vector<node<T> *> &v)
{
    for (uint i = 0; i < v.size(); i++)
        (bstroot.push_back(v[i])); // just copying the pointers to the nodes
}
